...
public class QuartzHostedService : IHostedService
{
    ...
    public async Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        var jobDetail = JobBuilder.Create<ResetChecklistsJob>()
            .WithIdentity("ResetListingsJob", "group1")
            .Build();

        var jobScheduleConfig = _configuration.GetSection("ResetListingsJobSchedule");
        int hour = jobScheduleConfig.GetValue<int>("HourOfTheDay");
        int minute = jobScheduleConfig.GetValue<int>("MinuteOfTheDay");

        var trigger = TriggerBuilder.Create()
            .WithIdentity("ResetListingsJobTrigger", "group1")
            .StartNow()
